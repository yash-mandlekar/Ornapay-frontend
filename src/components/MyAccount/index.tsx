...
import { User, Package, Download, MapPin, Settings, LogOut, Mail, Phone, Edit2, Save } from 'lucide-react';
...
const MyAccount = () => {
  ...
  const user = {
    avatar: "https://res.cloudinary.com/dpchknrfk/image/upload/v1763292491/avatars/k11bppfozqaw90wvhrrg.webp",
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, #f5f1ed 0%, #ede8e3 100%)', minHeight: '100vh' }}>
      {/* Breadcrumb */}
      <div style={{ backgroundColor: '#f5f1ed', borderBottom: '1px solid #e5d9d0' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <h1 style={{ color: '#4F1719', fontSize: '2rem', fontWeight: '600' }}>My Account</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-80 w-full">
            <div className="bg-white rounded-2xl  overflow-hidden" style={{ border: '1px solid #832729' }}>
              {/* User Profile Section */}
              <div style={{ backgroundColor: '#f5f1ed', padding: '2rem', borderBottom: '1px solid #e5d9d0' }}>
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full overflow-hidden" style={{ border: '3px solid #832729' }}>
                    <img 
                      src={user.avatar} 
                      alt={user.name}
                  ...
                  <div>
                    <h3 style={{ color: '#4F1719', fontWeight: '600', fontSize: '1.125rem' }}>
                      {user.name}
                    </h3>
...
const DashboardTab = ({ user, cartItems }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl  p-8" style={{ border: '1px solid #832729' }}>
        <h2 style={{ color: '#4F1719', fontSize: '1.875rem', fontWeight: '600', marginBottom: '1.5rem' }}>
      ...
      {cartItems.length > 0 && (
        <div className="bg-white rounded-2xl  overflow-hidden" style={{ border: '1px solid #832729' }}>
          <div style={{ backgroundColor: '#f5f1ed', padding: '1.5rem', borderBottom: '1px solid #e5d9d0' }}>
            <h3 style={{ color: '#4F1719', fontSize: '1.25rem', fontWeight: '600' }}>Current Cart Items</h3>
          </div>
          <div className="p-6 space-y-4">
            {cartItems.map((item, index) => (
              ...
              >
                ...
                />
                <div className="flex-1">
                  <h4 style={{ color: '#4F1719', fontWeight: '600', marginBottom: '0.25rem' }}>
                    {item.product.title}
                  </h4>
                  {item.variant && (
                    <p style={{ color: '#666666', fontSize: '0.875rem' }}>
                      {item.variant.attributes.map(attr => `${attr.key}: ${attr.value}`).join(', ')}
                    </p>
                  )}
                  <div className="flex items-center gap-4 mt-2">
                    <p style={{ color: '#832729', fontWeight: '600' }}>
                      ₹{(item.variant?.discountPrice || item.product.discountPrice).toLocaleString()}
                    </p>
                    <p style={{ color: '#666666', fontSize: '0.875rem' }}>
                      Qty: {item.quantity}
                    </p>
...
const DashboardCard = ({ icon: Icon, title, value, color }) => (
  ...
  >
    ...
    <h3 style={{ color: '#4F1719', fontSize: '1.5rem', fontWeight: '600', marginTop: '1rem' }}>
      {value}
    </h3>
...
const OrdersTab = () => (
  <div className="bg-white rounded-2xl  overflow-hidden" style={{ border: '1px solid #832729' }}>
    <div style={{ backgroundColor: '#f5f1ed', padding: '1.5rem', borderBottom: '1px solid #e5d9d0' }}>
      <h2 style={{ color: '#4F1719', fontSize: '1.5rem', fontWeight: '600' }}>My Orders</h2>
    </div>
...
const DownloadsTab = () => (
  <div className="bg-white rounded-2xl  p-8" style={{ border: '1px solid #832729' }}>
    <h2 style={{ color: '#4F1719', fontSize: '1.5rem', fontWeight: '600', marginBottom: '1.5rem' }}>
      Downloads
    </h2>
    <p style={{ color: '#666666', textAlign: 'center', padding: '3rem' }}>
      ...
      <div className="bg-white rounded-2xl  p-8" style={{ border: '1px solid #832729' }}>
        ...
        <div className="space-y-5">
          <div className="grid md:grid-cols-2 gap-5">
            <div>
              <label style={{ color: '#4F1719', fontWeight: '600', display: 'block', marginBottom: '0.5rem' }}>
